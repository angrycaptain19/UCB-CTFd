        ...
        if is_admin() and request.args.get("view") == "admin":
            ...
            challenges = (
                Challenges.query.filter(
                    and_(Challenges.state != "hidden", Challenges.state != "training" , Challenges.state != "locked")
                )
...
# http://localhost:8000/api/v1/challenges/attempt
# {challenge_id: 7, submission: "UCB{jajjaja}"}

@challenges_namespace.route("/training-attempt")
class ChallengeTrainingAttempt(Resource):
...
@challenges_namespace.route("/training")
class ChallengeTraining(Resource):
    ...
    def get(self, query_args):
        ...
        if is_admin() and request.args.get("view") == "admin":
            ...
            challenges = (
                Challenges.query.filter(
                    and_(Challenges.state != "hidden", Challenges.state != "visible" , Challenges.state != "locked")
                )
                .filter_by(**query_args)
                .filter(*filters)
                .order_by(Challenges.value)
